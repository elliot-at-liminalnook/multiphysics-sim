...
const planPath = `${root}/gentler-direct-plan.json`, plan = read(planPath);
const statusPath = `${root}/gentler-direct-status.json`, status = read(statusPath);
const integrityPath = `${root}/gentler-direct-integrity.json`, integrity = read(integrityPath);
...
const cases = status.cases.map((c, i) => {
  ...
  const config = structuredClone(r.recording.config);
  {
    ...
    assert.deepEqual(config.policy.step_reference.sequence.phase_durations_s, def.phase_durations_s);
  ...
  for (const [step, p] of landings(r)) {
  ...
  const prefix = `${root}/gentler-direct-${c.name}`, outcome = captureOutcome(r);
  ...
  return {name: c.name, completed: r.completed, error: c.error, outcome, task_passed: c.passed, acceptance: c.acceptance,
    slip_speed_m_s: def.slip_speed_m_s, baseline: def.baseline,
    matched_baseline_scene_and_other_config_identical: true,
...
writeFileSync(`${root}/gentler-direct-summary.json`, JSON.stringify({version: 1, cases,
  ...
  scope: 'Matched gentler direct-transfer development cases. Identical scene and controller/config except the two declared sequence fields; planned forward foot landings checked. Explicit separate smoothing profiles. Sampled reference acceleration is not actual body acceleration. Task and 5% contact screen unchanged; no browser, timestep, fresh held-out or terrain qualification.'}, null, 2) + '\n');
