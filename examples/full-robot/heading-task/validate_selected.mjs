...
const [run]=process.argv.slice(2);assert(run,'usage: validate_selected.mjs NEW-output-directory');
mkdirSync(run);const root='examples/full-robot/heading-task';
const read=p=>JSON.parse(readFileSync(p)),hash=p=>createHash('sha256').update(readFileSync(p)).digest('hex');
const recipe=read(`${root}/search.recipe.json`),reserved=read(`${root}/validation-plan.json`);
const cases=[...recipe.cases.map(c=>({...c,config:`${root}/${c.name}.config.json`,reserved:false})),
...
for(const c of cases){
 ...
 writeFileSync(`${root}/validation-status.json`,JSON.stringify({version:1,complete:reports.length===cases.length,
