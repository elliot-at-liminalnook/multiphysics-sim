...
const feet = markers.map(marker => {
  ...
  const samples = r.frames.map(frame => recordedContactMotion(frame, index, marker.link));
...
writeFileSync(output, JSON.stringify({version: 1, outcome, feet,
  capture: source(capturePath), sources: [import.meta.filename, 'examples/interactive/capture_outcome.mjs', 'examples/interactive/recorded_contact_motion.mjs'].map(source),
