...
const [run]=process.argv.slice(2);assert(run,'usage: collect_search.mjs completed-search-directory');
const output='examples/full-robot/heading-task',read=p=>JSON.parse(readFileSync(p));
...
for(const [j,name] of ['short','sustained','refined-minute'].entries()){
...
for(const c of read(`${output}/validation-plan.json`).cases){
 const config=read(c.config);config.policy.neural_residual=policy;
 write(`${c.name}.selected.config.json`,config);
}
const inputs=[recipe.scene,...recipe.cases.flatMap(c=>[c.config,c.task,c.actions]),`${output}/validation-plan.json`,
 ...read(`${output}/validation-plan.json`).cases.map(c=>c.config)];
