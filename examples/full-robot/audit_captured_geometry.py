...
p.add_argument("--probe-both-sides", action="store_true", help="Also classify each witness against its exact source solids")
a = p.parse_args()
print(json.dumps(audit_captured_pair(a.cad, json.loads(a.scene.read_text()),
    ...
    measure_pair_distances=not a.points_only, probe_both_sides=a.probe_both_sides), indent=2))
