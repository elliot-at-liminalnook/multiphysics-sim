...
const sdirk2=process.argv.includes("--sdirk2");
const exactProbeBase=process.argv.includes("--exact-probe-base");
...
fs.writeFileSync(output,JSON.stringify({version:1,timestep_reference:timestepReference,numerical_precision:numericalPrecision,block_factorization:blockFactorization,guarded_backtracking:guardedBacktracking,broyden_updates:broydenUpdates,linearized_probes:linearizedProbes,velocity_seed:velocitySeed,...(exactProbeBase?{exact_probe_base:true}:{}),...(sdirk2?{sdirk2:true}:{}),baseline:{path:baselinePath,sha256:hash(baselinePath)},candidate:{path:candidatePath,sha256:hash(candidatePath)},markers:{path:markerPath,sha256:hash(markerPath)},frames:a.frames.length,metrics,contact_pair_mismatches,phase_mismatches,
 scope:'Whole trajectories at common 50 Hz reporting times with identical scene, task and controls; configurations may differ only in derivative-reuse flags and, when explicitly requested, physics timestep/report stride, absolute/relative Newton tolerances independent-block factorization, guarded backtracking bounded Broyden updates linearized derivative probes or a guarded temporal velocity seed (and exact probe-base reuse when explicitly requested). SDIRK2 may differ only when explicitly requested. Contact forces aggregated by physical link pair. Differences include any altered subdivision sequence. This report measures differences without declaring them acceptable; independent task gates, between-sample contact impulses and hardware accuracy remain separate.'},null,2)+'\n');
