...
assert(['screen8','human20'].includes(profile));
...
const sourceCapture=read(spec.source_prefix+'.native.json');
...
assert.equal(spec.problem.constraints.length,profile==='screen8'?8:9);
const inputs=[spec.source_prefix+'.scene.json',spec.source_prefix+'.config.json',spec.source_prefix+'.actions.json',spec.source_prefix+'.native.json',spec.task];
const code=['examples/full-robot/contact-planning/analyze_controller_screen.mjs','examples/full-robot/speed-ceiling/analyze_validation.mjs','examples/full-robot/contact-planning/audit_contact_controller.mjs','examples/interactive/recorded_contact_motion.mjs',import.meta.filename];
...
function evaluate(values,method){
 ...
 if(!capture.completed||capture.error!==null){
  ...
  write(prefix+'.clearance.spec.json',{prefix,windows_s:profile==='screen8'?[[.8,3],[4.2,6.4]]:[[.8,6],[10.2,16]]});
...
const common=[evaluate(spec.baseline_values,'baseline-replay')];
