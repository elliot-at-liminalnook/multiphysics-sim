...
pub struct BuildOptions {
    ...
    /// Validation reference: difference the complete implicit residual instead
    /// of using component Jacobians or their sparsity pattern.
    pub numerical_jacobian: bool,
    /// Opt into experimental articulated exact/hybrid derivatives.
    pub hybrid_articulated_jacobian: bool,
    /// Experimental exact rate partials with compiler FD state partials.
    pub articulated_rate_partials: bool,
    /// Experimental central state probes for angular loop rows (relative radius).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub constraint_state_step: Option<f64>,
    /// Experimental exact loop identities; recorded so baseline comparisons
    /// can independently switch off this algebraic simplification.
    pub structural_loop_identities: bool,
    /// Experimental branch-local motor derivatives; physical law is unchanged.
    pub analytic_motor_jacobian: bool,
    /// Experimental explicit backlash engagement events with branch-local
    /// continuation during event location. No smoothing of the branch laws.
    pub backlash_events: bool,
    /// Recorded physical approximation, distinct from numerical optimizations.
    /// Retains source CAD parameters and the shared firmware/driver path.
    #[serde(default, skip_serializing_if = "sim_domain_robot::motor::MotorDynamics::is_detailed")]
    pub motor_dynamics: sim_domain_robot::motor::MotorDynamics,
    /// Experimental stale-matrix reuse across nearby event-location trials.
    pub event_jacobian_reuse: bool,
    /// Experimental guarded halving search; exhaustive Newton search is the
    /// reference. Recorded with the scene and shared by native/WASM execution.
    pub guarded_backtracking: bool,
}
impl Default for BuildOptions {
    fn default() -> Self {
        Self { planar: false, flex: true, contact: true, floor_friction: FloorFrictionModel::Bristle, step: 5.0e-4, sample: 0.01, seconds_hint: 2.0, verbose: false, report: 0.1, flex_modes: 4, driver_control: false, numerical_jacobian: false, hybrid_articulated_jacobian: false, articulated_rate_partials: false, constraint_state_step: None, structural_loop_identities: false, analytic_motor_jacobian: false, backlash_events: false, motor_dynamics: Default::default(), event_jacobian_reuse: false, guarded_backtracking: false }
    }
}
...
impl PhysicalRobot {
    ...
    /// Reconstruct the articulated state AND rates at an audited implicit
    /// stage. No reporting-interval acceleration estimate or live-state fallback.
    pub fn generalized_at_solver_point(&self, island_index: usize, point: &sim_dynamics::ImplicitAttempt) -> Result<Option<Generalized>, String> {
        let island = self.runtime.islands.get(island_index).ok_or("invalid island")?;
        let system = &island.system;
        if point.stage_state.len() != system.full_of.len() || point.stage_rate.len() != system.full_of.len() {
            return Err("solve-point dimension does not match island".into());
        }
        let (x, rates) = system.expand_at(point.stage_time, &point.stage_state, &point.stage_rate);
        let values: std::collections::HashMap<_,_> = system.state_ids.iter().copied().zip(x.into_iter().zip(rates)).collect();
        if self.art_states.first().is_none_or(|id| !values.contains_key(id)) { return Ok(None); }
        let read = |id: &StateId| values.get(id).copied().ok_or_else(|| format!("physical channel {id:?} missing from solver point"));
        let pairs: Vec<_> = self.art_states.iter().map(read).collect::<Result<_,_>>()?;
        let mut angles = vec![0.0];
        angles.extend(self.port_angles.iter().map(|id| read(id).map(|v| v.0)).collect::<Result<Vec<_>,_>>()?);
        let temps = self.temperature_ids.iter().map(|id| read(id).map(|v| v.0)).collect::<Result<Vec<_>,_>>()?;
        Ok(Some(self.art.generalized(pairs.iter().map(|p| p.0).collect(), pairs.iter().map(|p| p.1).collect(), &angles, temps)))
    }
...
pub fn perturb(model: &PhysicalModel, rng: &mut Rng) -> PhysicalModel {
    ...
    for j in &mut m.joints {
        ...
        let backlash_scale = (1.0 + rng.normal() * u.backlash.sigma_fraction).max(0.0);
        if let Some(value) = j.physics.drive_backlash.as_ref().and_then(|b| b.width_rad) {
            j.physics.set_drive_backlash_rad(value * backlash_scale, "Monte Carlo perturbation of authored drive backlash".into());
        } else if j.physics.drive_backlash.is_none() { j.physics.backlash *= backlash_scale; }
        let sf = (1.0 + rng.normal() * u.stiffness.sigma_fraction).max(0.1);
...
fn apply_fit(model: &PhysicalModel, x: &[f64], joints: &[String], motors: &[String]) -> PhysicalModel {
    ...
    for (k, jname) in joints.iter().enumerate() {
        if let Some(j) = m.joints.iter_mut().find(|j| &j.name == jname) {
            ...
            j.physics.set_drive_backlash_rad(x[4 * k + 2].abs(), "system-identification optimization candidate".into());
            let s = x[4 * k + 3].exp();
...
pub fn fit(model: &PhysicalModel, log: &Log, log_path: &str, opts: &BuildOptions, max_iterations: usize) -> Result<Value, String> {
    ...
    for jname in &joints {
        ...
        x0.extend([j.physics.friction.coulomb.max(1e-5).ln(), j.physics.friction.viscous.max(1e-5).ln(), j.physics.drive_backlash_rad(model.version >= 4)?, 0.0]);
    }
    for _ in &motors {
        x0.push(0.0);
    }
    ...
    for (k, jname) in joints.iter().enumerate() {
        ...
        let mut entry = json!({
            ...
            "backlash": j.physics.drive_backlash_rad(fitted.version >= 4)?,
