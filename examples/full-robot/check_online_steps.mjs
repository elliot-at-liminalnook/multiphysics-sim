...
if(capture.recording.scene.options.omit_inter_link_contact){
 assert.equal(batch.inter_link_geometry_audit?.frames,capture.frames.length,'omitted forces require complete independent geometry audit');
 assert(Number.isFinite(batch.inter_link_geometry_audit.penetrating_samples));
}
...
const internal=Math.max(capture.frames.reduce((n,f)=>n+f.contacts.filter(c=>c.other!=null).length,0),batch.inter_link_geometry_audit?.penetrating_samples??0);
