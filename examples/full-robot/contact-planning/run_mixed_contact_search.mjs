...
for(let generation=0;generation<spec.generations;generation++) {
  ...
  for(let i=0;i<batch.points.length;i++) {
    ...
    let recipe=read(template.path);
    const index=kind=>recipe.variables.findIndex(v=>v.decision.kind==='motion'&&v.decision.decision.kind===kind);
    const value=(b,x)=>b.lower+x*(b.upper-b.lower);
    const overrides=['period','displacement_along_direction'].map((kind,j)=>{
      const i=index(kind);assert(i>=0);return [i,value(recipe.variables[i].bound,point.continuous[j])];
    });
    write(prefix+'.overrides.json',overrides);
    const edit=run(prefix+'-initializer',spec.tools.initializer.path,[template.path,prefix+'.overrides.json']);
    // Invalid configuration is an orchestration error, not a gait measurement.
    assert.equal(edit.exit_code,0,'shared initializer rejected the trial; preserve batch and logs');
    recipe=read(prefix+'-initializer.stdout.json');
