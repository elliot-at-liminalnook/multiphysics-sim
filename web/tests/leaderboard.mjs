...
try {
  ...
  const shortest = data.entries.find(e => e.id === 'fast-distilled-steering') ?? data.entries.find(e => e.id === 'exact-base-steering') ?? data.entries.find(e => e.id === 'portable-tangent-steering') ?? data.entries.find(e => e.id === 'tangent-steering-short') ?? data.entries.find(e => e.id === 'secant-steering-short') ?? data.entries.find(e => e.id === 'faster-steering-short') ?? [...data.entries].sort((a, b) => a.replay.completed_steps - b.replay.completed_steps)[0];
