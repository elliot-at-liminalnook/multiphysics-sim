...
const root = 'examples/full-robot/whole-swing', run = 'runs/interactive/direct-support';
...
const statusPath = `${root}/direct-support-browser-status.json`, status = read(statusPath);
const nativePath = `${root}/direct-support-status.json`, native = read(nativePath);
...
const cases = status.cases.map(c => {
  ...
  const n = native.cases.find(n => n.name === ({'direct-turn': 'direct-steering'})[c.name]);
...
const report = {version: 1, passed: true, cases, ui,
  ...
  scope: 'Rendered direct-transfer steering recording exactly matches the accepted native recipe, seed and inputs. Steering native/WASM comparison and exact replay/reset pass. Full-minute browser timing and portability are unmeasured; native minute fails heading and contact-motion gates. Every leaderboard load, tested steering replay, video export, mobile layout and tamper rejection is checked. Native task success and failed browser timing remain separate.'};
writeFileSync(`${root}/direct-support-browser-integrity.json`, JSON.stringify(report, null, 2) + '\n');
